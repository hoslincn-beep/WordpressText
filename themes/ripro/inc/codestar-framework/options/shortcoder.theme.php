...
function shop_shortcode($atts, $content)
{
    ...
    switch ($RiProPayAuth->ThePayAuthStatus()) {
      ...
      case 13: //免登陆 输出购买按钮信息
        ...
        $do_shortcode .= '<div class="login-false">'.$close_novip_pay_str.'当前隐藏内容需要消耗';
        $do_shortcode .= '<div class="coin"><span class="label label-warning">' . $cao_this_am . '</span></div>';
        $do_shortcode .= '</div>';
        $do_shortcode .= '<p class="t-c">已有<span class="red">' . $cao_paynum . '</span>人下载</p>';
        $do_shortcode .= '<div class="pc-button">';
      ...
      case 22: //登陆后  输出购买按钮信息
        ...
        if ($cao_close_novip_pay && !$CaoUser->vip_status()) {
            ...
            $do_shortcode .= '<button type="button" class="click-pay btn btn--secondary" data-postid="' . $post_id . '" data-nonce="' . $create_nonce . '" data-price="' . $cao_this_am . '"><i class="fa fa-money"></i> 立即下载</button>';
        }
        $do_shortcode .= '</div>';
        $do_shortcode .= '</div>';
        break;
      case 31: //没有开启免登录 没有登录 输出登录后进行操作
        $do_shortcode = '<div class="content-hide-tips"><i class="fa fa-lock"></i>';
        $do_shortcode .= '<span class="rate label label-warning">' . $rate_text . '</span>';
        $do_shortcode .= '<div class="login-false">当前隐藏内容需要消耗';
        $do_shortcode .= '<div class="coin"><span class="label label-warning">' . $cao_this_am . '</span></div>';
        $do_shortcode .= '</div>';
        //以下IF ELSE内容是我自己改的。未登录用户，收费资源即$cao_paynum > 0，则输出价格和有多少人下载，免费资源则输出阅读人数。
        if($cao_paynum >= 1){
           $do_shortcode .= '<p class="t-c">已有<span class="red">' . $cao_paynum . '</span>人下载</p>';
           $do_shortcode .= '<div class="pc-button">';
        }else{
        ...
        $do_shortcode .= '<div class="pc-button">';
        $do_shortcode .= '<button type="button" class="login-btn btn btn--primary"><i class="fa fa-user"></i> 登录下载</button>';
        $do_shortcode .= '</div>';
        $do_shortcode .= '</div>';
        break;
    }
