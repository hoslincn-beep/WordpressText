...
foreach ($mode_catpost['catcms'] as $key => $cms) {
    ...
    $child_categories = get_terms('category', array('hide_empty' => 0, 'parent' => $category->cat_ID, 'orderby' => 'count', 'order' => 'DESC'));
    ?>

    <div class="section pb-0">
        <div class="container">
            <h3 class="section-title">
                <span><i class="fa fa-bar-chart"></i> <a
                            href="<?php echo esc_url(get_category_link($category->cat_ID)); ?>"><?php echo $category->cat_name; ?></a></span>
                <div class="choosetype">
                    <?php
                    if (isset($child_categories)) {
                        foreach ($child_categories as $k => $items) {
                            echo ' <a href="'.esc_url(get_category_link($items->term_id)).'" target="_blank">'.$items->name.'</a>';
                        }
                    }
                    ?>
                </div>
